...
public class ParametricBuilding extends Building implements Cloneable {
	...
	public ParametricBuilding(TopologieBatiment tB, double largeur, double hauteur, double largeur2, double hauteur2,
			double zGouttiere, double zMax, Materiau materiauToit, List<Materiau> materiauFacades,
			boolean[] facadesNonAveugles, IDirectPosition centre, double angle, double angleToit) {
		this(tB, largeur, hauteur, largeur2, hauteur2, zGouttiere, zMax, materiauToit, materiauFacades,
				facadesNonAveugles, centre, angle, null, angleToit);

	}

	public ParametricBuilding(TopologieBatiment tB, double largeur, double hauteur, double largeur2, double hauteur2,
			double zGouttiere, double zMax, Materiau materiauToit, List<Materiau> materiauFacades,
			boolean[] facadesNonAveugles, IDirectPosition centre, double angle, DTM mnt, double angleToit) {
		super();
		this.tB = tB;
		this.largeur = largeur;
		this.hauteur = hauteur;
	...
	public boolean generationEmprise() {
		...
		IPolygon emprise = GenerateEmprise.generateEmprise(tB.getfE(), largeur, hauteur, largeur2, hauteur2);

		// 2 on tourne l'emprise
		try {
			emprise = (IPolygon) JtsGeOxygene
					.makeGeOxygeneGeom(JtsUtil.rotation((Polygon) JtsGeOxygene.makeJtsGeom(emprise), angle));
		} catch (Exception e) {
	...
	public boolean generationToit() {
		...
		ToitProcedural t = GenerationToit.generationToit(tB, this.centre.getZ() + zGouttiere, this.centre.getZ() + zMax,
				materiauToit, this.getFootprint(), angleToit);
	...
	public boolean generationFacade() {
		...
		List<FacadeProcedural> lF = GenerationFacade.generate(this.getRoof(), materiauFacades, this.centre.getZ(),
				facadesNonAveugles);
	...
	public ParametricBuilding clone() {
		...
		ParametricBuilding pB = new ParametricBuilding(this.tB, this.largeur, this.hauteur, this.largeur2,
				this.hauteur2, this.zGouttiere, this.zMax, this.materiauToit, this.materiauFacades,
				this.facadesNonAveugles, this.centre, this.angle, this.angleToit);
		pB.generateAll();
		return pB;
